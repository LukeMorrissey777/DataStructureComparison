...
int main(int argc, char** argv)
{
    ...
    //and search times as well as a LL
    int count = 0;
    int numDat = 0;
    float insert[400];
    float search[400];
    ...
    while(count < 400)
    {
        ...
        for(int i = 0; i < 100;i++)
        {
            //get random data we've already inserted and search for it
            int x = rand()%numel;
    ...
    //prints last insert and search times
    ...
    std::ofstream insertTimes("LLi.csv");
    for(int i = 0; i < 399; i++)
    {
    ...
    std::ofstream searchTimes("LLs.csv");
    for(int i = 0; i < 399; i++)
    {
